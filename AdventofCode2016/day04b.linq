...
rwcnawjcrxwju-ljwmh-bqryyrwp-277[nxatm]";
	...
	foreach (var room in input.Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries))
	{
		var check_sum = checksumRegex.Match(room).Value;
		var sector_id = Convert.ToInt32(sectorIdRegex.Match(room).Value);
		var room_name = roomNameRegex.Match(room).Value;

		var room_chars = room_name.ToCharArray();
		var distinct_chars = room_chars.Distinct().Except(new[] { '-' }).ToList();
		var occs = distinct_chars.ToDictionary(c => c, c => room_chars.Count(x => x.Equals(c)));

		if (new string(occs.OrderByDescending(x => x.Value).ThenBy(x => x.Key).Select(x => x.Key).Take(5).ToArray()) ==
			check_sum)
		{
			var decrypted_name = new string(room_name.Select(c => c.Equals('-') ? ' ' : rotate(c, sector_id)).ToArray());
