...
public static bool checkLineTLS(string l)
{
	foreach (Match bracketed in Regex.Matches(l, @"(?<=\[)\w+(?=\])"))
	{
		if (Regex.IsMatch(bracketed.Value, @"(.)(.)\2\1"))
			return false;
	}

	string[] nonBracketed = Regex.Split(l, @"\[[^\]]*\]");
	foreach (var v in nonBracketed)
	{
		if (Regex.Match(v, @"(.)((?!\1).)(\2)(\1)").Success)
			return true;
	}
	return false;
}

static bool checkLineSSL(string l)
{
	List<string> aba = new List<string>();
	List<string> aba2 = new List<string>();

	foreach (string ip in Regex.Split(l, @"\[[^\]]*\]"))
	{
		foreach (Match m in Regex.Matches(ip, @"(.)((?!\1).)(\1)"))
		{
			if (m.Value[0] != m.Value[1])
				aba.Add(m.Value);
		}

		aba2.AddRange(checkABA(ip));
	}

	//if (aba.Count != aba2.Count)
	//	return false;

	foreach (var val in aba)
	{
		string bab = val[1].ToString() + val[0].ToString() + val[1].ToString();
		foreach (Match m in Regex.Matches(l, @"\[(\w*)\]"))
		{
