...
Sue 500: perfumes: 4, cars: 9, trees: 4";
	...
	foreach (var l in input.Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries))
	{
		var sue = new AuntSue();
		var items = l.Replace(",", "").Replace(":", "").Split();
		...
		for (var i = 0; i < 8; i += 2)
		{
			...
			{
				case "Sue":
					sue.Number = Int32.Parse(items[i + 1]);
					break;
				case "children":
					sue.Children = Int32.Parse(items[i + 1]);
					break;
				case "cats":
					sue.Cats = Int32.Parse(items[i + 1]);
					break;
				case "samoyeds":
					sue.Samoyeds = Int32.Parse(items[i + 1]);
					break;
				case "pomeranians":
					sue.Pomeranians = Int32.Parse(items[i + 1]);
					break;
				case "akitas":
					sue.Akitas = Int32.Parse(items[i + 1]);
					break;
				case "vizslas":
					sue.Vizslas = Int32.Parse(items[i + 1]);
					break;
				case "goldfish":
					sue.Goldfish = Int32.Parse(items[i + 1]);
					break;
				case "trees":
					sue.Trees = Int32.Parse(items[i + 1]);
					break;
				case "cars":
					sue.Cars = Int32.Parse(items[i + 1]);
					break;
				case "perfumes":
					sue.Perfumes = Int32.Parse(items[i + 1]);
					break;
			}
		}
	...
	sues.Where(s => s.Children.HasValue ? s.Children.Value == 3 : true)
		.Where(s => s.Cats.HasValue ? s.Cats.Value > 7 : true)
		.Where(s => s.Samoyeds.HasValue ? s.Samoyeds.Value == 2 : true)
		.Where(s => s.Pomeranians.HasValue ? s.Pomeranians.Value < 3 : true)
		.Where(s => s.Akitas.HasValue ? s.Akitas.Value == 0 : true)
		.Where(s => s.Vizslas.HasValue ? s.Vizslas.Value == 0 : true)
		.Where(s => s.Goldfish.HasValue ? s.Goldfish.Value < 5 : true)
		.Where(s => s.Trees.HasValue ? s.Trees.Value > 3 : true)
		.Where(s => s.Cars.HasValue ? s.Cars.Value == 2 : true)
		.Where(s => s.Perfumes.HasValue ? s.Perfumes.Value == 1 : true)
		.Dump();
}
